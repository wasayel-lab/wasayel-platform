...
public sealed class TenantPlan
{
    ...
    // ═══ PayPal — ثَلاثَةُ حُقولٍ، ولا واحِدٌ مِنها يَدخُل القَرار ═════
    //
    // **والتَكافُؤُ الصِفريُّ يَبقى كَما هُوَ**: كُلُّها `null`
    // افتِراضاً، و<see cref="TenantPlanPolicy.Derive"/> لا تَقرَأ
    // مِنها حَرفاً. فَالحالَةُ ما زالَت مُشتَقَّةً مِن
    // `Status` والتَواريخِ وَحدَها — وPayPal **يُحَرِّكُ تاريخاً**،
    // ولا يُضيف مَصدَرَ حَقيقَةٍ ثانِياً.

    /// <summary>مُعَرِّفُ الاشتِراكِ في PayPal — يُملَأُ مِن أَوَّلِ
    /// رِسالَةٍ مُوَثَّقَة، ولِلتَدقيقِ ولِإلغاءٍ يَدَوِيٍّ مِن
    /// اللَوحَة.</summary>
    public string? PayPalSubscriptionId { get; set; }

    /// <summary>رابِطُ المُوافَقَةِ كَما أَعادَتهُ PayPal عِندَ
    /// الإنشاء — يَنسَخُه المُشرِفُ ويُرسِلُه، ويَراهُ رائِدُ
    /// الأَعمالِ في الاستوديو. <b>يُخَزَّن لِأَنّ الطَرَفَينِ
    /// يَحتاجانِه</b>، ولَو بَقِيَ في صَفحَةِ نَتيجَةٍ واحِدَةٍ
    /// لَضاعَ بِتَحديثِ الصَفحَة.</summary>
    public string? PayPalApproveUrl { get; set; }

    /// <summary>
    /// <para>مَتى وَصَلَت رِسالَةُ إلغاءٍ أَو تَعليقٍ مِن PayPal.</para>
    ///
    /// <para><b>ولا تُطفِئ مَتجَراً</b>: الحالَةُ تَبقى مُشتَقَّةً مِن
    /// الوَقت، فَمَن دَفَعَ مُدَّتَه يَأخُذُها كامِلَةً ثُمَّ يَنتَهي
    /// كَأَيّ مُنتَهٍ. وهذا الحَقلُ <b>عَلامَةُ عَرضٍ وتَدقيق</b> —
    /// «لَن يُجَدَّد تِلقائِيّاً» — لا مِفتاحُ إغلاق. وإطفاءُ مَتجَرٍ
    /// دَفَعَ ثَمَنَ شَهرِه لِأَنَّه أَلغى التَجديدَ في يَومِه الثاني
    /// مُصادَرَةٌ لا سِياسَة.</para>
    /// </summary>
    public DateTime? RenewalCancelledAt { get; set; }
}

// ═══ مَرَّة-واحِدَة لِرِسائِل PayPal ═════════════════════════════════

/// <summary>
/// <para><b>سِجِلُّ الأَحداثِ المُطَبَّقَة</b> — مُعَرِّفُه
/// <c>event_id</c> مِن PayPal. والتَفَرُّدُ مِن <b>مِفتاحِ الوَثيقَةِ
/// نَفسِه</b> لا مِن فَحصِ وُجودٍ في التَطبيق: إدراجٌ ثانٍ يَرتَدُّ مِن
/// Postgres. نَفسُ حُجَّةِ <c>ApiIdempotencyRecord</c> حَرفاً.</para>
///
/// <para><b>وهُوَ يُدرَج في نَفسِ جَلسَةِ التَمديدِ ونَفسِ
/// الإيداع</b> — أَي <b>مُعامَلَةٌ واحِدَة</b>. وهذا هُوَ الشَكلُ
/// الَّذي وَصَفَته وَثيقَةُ سَطحِ الـAPI وتَعَذَّرَ عَلَيها هُناك
/// (‏<c>DealsService</c> تَفتَح جَلسَتَها بِنَفسِها فَلَزِمَت
/// مُعامَلَتان). وهُنا لا مانِع: القَرارُ دالَّةٌ نَقِيَّةٌ والأَثَرُ
/// خِدمَةٌ تَأخُذ الجَلسَة — <b>فَإمّا يُمَدَّدُ ويُسَجَّلُ المِفتاحُ
/// مَعاً، أَو لا يَقَعُ شَيء</b>، ولا نافِذَةَ <c>in_progress</c>
/// أَصلاً.</para>
///
/// <para><b>و<c>SingleTenanted</c></b> كَـ<see cref="TenantPlan"/>
/// ولِنَفس السَبَب: يُقرَأُ ويُكتَب في نُقطَةٍ **بِلا مُستَأجِرٍ
/// مَحلول** (‏`api` مَحجوزَةٌ في وَسيطِ المُستَأجِر).</para>
/// </summary>
public sealed class PayPalWebhookRecord
{
    /// <summary><c>event_id</c> — المِفتاحُ الأَوَّليّ.</summary>
    public string Id { get; set; } = "";

    public string EventType { get; set; } = "";

    /// <summary>سلاجُ المُستَأجِرِ مِن <c>custom_id</c>.</summary>
    public string TenantSlug { get; set; } = "";

    /// <summary>ما فُعِل — اسمُ عُضوِ
    /// <c>PayPalBillingAction</c>. نَصٌّ لا تَعداد: الوَثيقَةُ تَعيش
    /// أَطوَلَ مِن أَسماءِ أَعضاءِ تَعدادٍ في كود.</summary>
    public string Action { get; set; } = "";

    /// <summary>تاريخُ الانتِهاءِ بَعدَ التَطبيق — فَيُقرَأُ مِن
    /// السِجِلِّ ماذا فَعَلَت الرِسالَةُ فِعلاً، لا ماذا كانَ
    /// يُفتَرَض.</summary>
...
/// <param name="PayPalPlanId">
/// <para><b>مُعَرِّفُ خُطَّةِ PayPal — اختِياريٌّ، وبِه وَحدَه يُعرَض
/// رابِطُ الدَفع.</b> غائِبٌ (‏<c>null</c>) ⇒ لا بِطاقَةَ PayPal في
/// <c>/admin</c> ولا في الاستوديو — <b>لا زِرَّ يَقول «قَريباً»</b>
/// (القاعِدَة ١٢).</para>
///
/// <para><b>ولِماذا هُنا لا في وَثيقَةِ المُستَأجِر</b>: الخُطَّةُ
/// خَصيصَةُ <b>الباقَة</b> لا خَصيصَةُ مَتجَر — كُلُّ مَن يَشتَري
/// «الباقَةَ الفُلانِيَّةَ» يَشتَري نَفسَ خُطَّةِ PayPal. ووَضعُه
/// عَلى المُستَأجِرِ كانَ سَيُنتِج نُسخَةً لِكُلّ مَتجَرٍ تَنجَرِف.</para>
///
/// <para><b>ويَبقى فارِغاً اليَوم</b>: يَملَؤُه المالِكُ يَومَ
/// يُنشِئ خُطَّتَه في PayPal بِسِعرِها — ولا سِعرَ يُكتَب هُنا ولا
/// يُخمَّن (القاعِدَة ١٦). <c>docs/DEPLOY.md</c> §٢·ج يَقولُ كَيف.</para>
/// </param>
public sealed record PlatformPlanDefinition(
    ...
    [property: JsonPropertyName("paypalPlanId")] string? PayPalPlanId = null);
