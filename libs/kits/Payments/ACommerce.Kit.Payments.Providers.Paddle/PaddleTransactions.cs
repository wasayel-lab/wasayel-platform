...
public static class PaddleTransactionStatuses
{
    ...
    ///
    /// <para><b>و«عادَ المال» يُبلَغ مِن كُلِّ حالَةٍ غَيرِ نِهائِيَّة،
    /// لا مِن «وَصَلَ المال» وَحدَها — وهذا ضِلعٌ كُتِبَ بِكُلفَةٍ
    /// مَقيسَة (‏<c>docs/ADR-011</c>).</b> كانَ الجَدوَلُ يَعرِف
    /// <c>(completed → refunded)</c> ولا يَعرِف
    /// <c>(created → refunded)</c>، وفَرعُ «لا يُسحَبُ ما لَم يُمنَح»
    /// يَطلُب <c>refunded</c> على مُعامَلَةٍ <c>created</c>. فَكانَ
    /// القَرارُ يَقول «تُعَلَّم» والحارِسُ يَرفُضُ <b>بِصَمت</b>،
    /// ويُرَدُّ ‏200 فَتَتَوَقَّفُ إعادَةُ Paddle، وتَبقى الحالَةُ
    /// <c>created</c>. ثُمَّ يَضبُطُ المُشرِفُ وَثيقَةَ الباقَةِ
    /// وتُعادُ رِسالَةُ «اكتَمَلَت» — <b>فَتُمَدَّدُ ثَلاثونَ يَوماً
    /// لِمالٍ عاد</b>.</para>
    ///
    /// <para><b>ولا يَفتَحُ هذا الضِلعُ بابَ السَحب</b>: حارِسُ
    /// <c>PaddleBillingPolicy.MayWriteStatus</c> يَشتَرِط
    /// <c>Withdraw</c> لِمَن <b>يُغادِرُ</b> <c>completed</c> وَحدَه،
    /// و<c>created</c> لَم تَبلُغها. والسَحبُ نَفسُه يَشتَرِط
    /// <c>completed</c> في القَرارِ قَبلَ الحارِس — فَما لَم يُمنَح لا
    /// يُسحَب، <b>ويُعَلَّم</b>.</para>
    ///
    /// <para><b>و<c>(canceled → refunded)</c> مَعَه بِنَفسِ
    /// الحُجَّة</b>: الفَرعُ نَفسُه يَطلُبُه، والحالَةُ اليَومَ
    /// <b>بِلا كاتِبٍ واحِدٍ في المُستَودَع</b> (مَقيس) — فَإغلاقُ
    /// الضِلعِ لا يُغَيِّرُ سُلوكاً قائِماً، ويَمنَعُ عَودَةَ نَفسِ
    /// العَطَبِ يَومَ يوجَدُ كاتِب. <b>وحارِسٌ بِلا استِثناءٍ أَقوى
    /// مِن حارِسٍ بِاستِثناءٍ مُوَثَّق</b> (القاعِدَة ٢).</para>
    /// </summary>
