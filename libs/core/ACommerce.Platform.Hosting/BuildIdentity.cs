...
/// إيداعٍ يَخدِمُه الـSpace. ‏<c>huggingface.co/api/spaces/…</c> يَصِفُ
/// <b>مُستودَعَ</b> الـSpace — و<c>sha</c> فيه إيداعُ المِرآةِ المُسَطَّحِ
/// لا إيداعُ المَصدَر — ولا يَقولُ أَيَّ بِناءٍ <b>يَخدِمُ الآن</b>؛
/// ورَأسُ <c>x-proxied-replica</c> يُثبِتُ <b>تَبَدُّلَ حاوِيَةٍ لا
/// هُوِيَّةَ بِناء</b>؛ و<c>runtime.stage</c> — بِنَصِّ وَظيفَةِ النَشرِ
/// نَفسِها — <b>خَبَرٌ لا بُرهان</b>.
/// <b>وتَصحيحٌ مَقيس</b>: «‏يَرُدُّ ‏401 بِلا رَمز» — كَما كُتِبَ هُنا
/// أَوَّلاً — <b>غَيرُ صَحيح</b>؛ قيسَ ‏200 بِلا رَمز، والـSpace عامّ.
/// التَفصيلُ في ‏ADR-019 §١.</para>
///
...
public static class BuildIdentity
{
        ...
        DateTimeOffset startedAt)
    {
        ...
        // ═══ ولِماذا HEAD مَعَ GET — عِلَّةٌ مَقيسَةٌ لا احتِياط ═══════
        //
        // ‏`MapGet` يُطابِقُ GET وَحدَه، فَـ`HEAD /health` كانَ يَسقُطُ
        // مِن هذِه النُقطَةِ عِندَ مُرَشِّحِ الطُرُق. **ولا يَنتَهي
        // بِـ405 في التَطبيقِ الحَقيقيّ**: الطَلَبُ يَنزِلُ إلى
        // `@page "/{slug}"` في `TenantHome.razor` فَيُرَدُّ ‏200
        // بِـ`text/html`. أَي أَنّ المُراقِبَ الَّذي يَبعَثُ HEAD — وهو
        // الافتِراضُ في أَكثَرِ أَدَواتِ المُراقَبَة — كانَ يَقرَأُ
        // صَفحَةَ مُستَأجِرٍ ويَحسِبُها النُقطَة: **يَطمَئِنُّ إلى شَيءٍ
        // لَيسَ الَّذي سَأَلَ عَنه**، وذاكَ أَسوَأُ مِن ‏405 لِأَنَّه
        // يَكذِبُ في اتِّجاهِ السَلامَة.
        //
        // والتَسجيلُ بِالطَريقَتَينِ يَجعَلُ المَقطَعَ الحَرفِيَّ
        // `/health` يَغلِبُ `/{slug}` في المُطابَقَةِ لِكِلَيهِما.
        // والبُرهانُ على الأُنبوبِ الكامِلِ في
        // `scripts/verify-production-boot.sh` (‏`HEAD|/health|200`)، لا
        // على مُضيفٍ مُصَغَّرٍ يُسَجِّلُ النُقطَةَ وَحدَها.
        endpoints.MapMethods("/health", new[] { HttpMethods.Get, HttpMethods.Head }, (HttpResponse response) =>
        {
