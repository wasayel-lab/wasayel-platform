...
public static class HostingExtensions
{
        ...
        builder.Services.AddMarten(opts =>
            {
                ...
                // ‏(‏رُفِعَ مِن هُنا يَومَ ‏2026-08-23 تَسجيلُ
                //   `SubscriptionRequest` — طَلَبُ اشتِراكِ **مُستَخدِمٍ في
                ...
                // وَثيقَةُ الدَفعِ المُعَلَّق — مُعَرِّفُها **مَرجِعُنا**
                // (‏`custom_id`) لا مُعَرِّفُ الطَلَبِ عِندَ PayPal:
                // الأَخيرُ لا يُعرَف إلّا بَعدَ النِداء، والمَرجِعُ
                // يُرسَل فيه. و`SingleTenanted` كَجاراتِه ولِنَفسِ
                // السَبَبِ حَرفاً: تُقرَأ وتُكتَب في نُقطَةٍ بِلا
                // مُستَأجِرٍ مَحلول (‏ADR-006).
    ...
    public static WebApplication UsePlatformHost(this WebApplication app)
    {
        ...
        // ═══ النِطاقُ الفَرعِيُّ يَصيرُ مَقطَعَ مَسارٍ — هُنا وَحدَه ═══
        //
        // **قَبلَ `UseStaticFiles` و`UseRouting` عَمداً**: الحَقنُ
        // بَعدَ التَوجيهِ يَصِلُ مُتَأَخِّراً بِنُقطَةٍ واحِدَةٍ
        // فَيُصَيِّرُ ‏404، وبَعدَ المَلَفّاتِ الساكِنَةِ يَقرَأُ
        // مَسارَ الأُصولِ مُحَوَّلاً. وبِلا نِطاقٍ أَساسٍ مُهَيَّأٍ
        // — وهُوَ الافتِراضيُّ اليَوم — **هذا السَطرُ عَبورٌ خالِص**.
        // التَفصيلُ في ADR-022.
        app.UseSubdomainTenantPath();
