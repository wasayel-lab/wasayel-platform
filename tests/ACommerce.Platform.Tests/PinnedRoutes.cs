...
internal static class PinnedRoutes
{
    ...
    internal static readonly string[] StoreTakers =
    {
        ...
        // ─── سَطرٌ يُضافُ بِقَرارٍ مَرئيّ، وهذا سَبَبُه ─────────────────
        // ‏`/studio/begin` صارَ يَستَأنِفُ التَحليلَ لِمَن دَخَلَ أَصلاً
        // (‏2026-08-30) — وكانَ يُحَوِّلُ إلى البابِ بِلا شَرط،
        // فَتُكتَبُ الكَعكَةُ ولا يَستَهلِكُها أَحَد.
        //
        // **وأَخذُهُ المَخزَنَ لَيسَ خِياراً هُنا بَل تَبَعِيَّة**:
        // الاستِئنافُ واحِدٌ (`ResumeStudioPromptAsync`) تَستَدعيهِ
        // النُقطَتانِ المُثَبَّتَتانِ فَوقَه بِتَوقيعِهِ نَفسِه، وهُوَ
        // يَفتَحُ جَلسَةَ المُستَأجِرِ الثابِتِ `_studio` — **لا
        // مُستَأجِرَ في المَسارِ يُشتَقُّ مِنه `IDocumentSession`**.
        // فَحَقنُ الجَلسَةِ هُنا يَعني جَلسَةَ مُستَأجِرٍ خاطِئ.
        //
        // والبَديلُ الصَحيحُ نَقلُ الاستِئنافِ الثُلاثيِّ كُلِّه إلى
        // خِدمَةٍ تَأخُذُ الجَلسَةَ — **مَوجَةٌ تَمَسُّ ثَلاثَ نِقاطٍ
        // مَعاً**، ولا تُخلَط بِإصلاحِ عَطبٍ حَيّ. فَالسَطرُ دَينٌ
        // يُتَّخَذ لا يَنزَلِق، ويُرفَع مَعَ أُختَيهِ في مَوجَتِهِنّ.
        "/studio/begin",
        ...
        // ─── ‏2026-08-30 (‏ADR-012): سَطرانِ بِنَفسِ الحُجَّةِ حَرفاً ──
        //
        // نُقطَتا رَبطِ المُزَوِّدِ وسَحبِه. وأَخذُهُما المَخزَنَ
        // **لِسَبَبَينِ مَقيسَين، لا إهمالاً**:
        //
        //   ١. الحارِس. `TenantAdminGuard.CanAdministerAsync` يَأخُذ
        //      `IDocumentStore` لِأَنَّه يَقرَأُ وَثيقَةَ المُستَأجِرِ
        //      **العامَّة** ثُمَّ مُستَخدِمَ المَتجَرِ في جَلسَتِه —
        //      إيجارانِ في قَرارٍ واحِد، ولا تَحمِلُهُما جَلسَةٌ واحِدَة.
        //      والبَديلُ تَعريفٌ ثانٍ لِلتَخويلِ يَنجَرِف (القاعِدَة ٨).
        //
        //   ٢. وَثيقَةُ `Tenant` نَفسُها. أَوَّلُ كاتِبٍ لِـ
        //      `PaymentProviderConfigured` يَكتُب في وَثيقَةٍ
        //      **غَيرِ مَحصورَةٍ بِإيجار** (`SingleTenanted`)، وجَلسَةُ
        //      الطَلَبِ مَحصورَةٌ بِسلاجِ المَسار.
        //
