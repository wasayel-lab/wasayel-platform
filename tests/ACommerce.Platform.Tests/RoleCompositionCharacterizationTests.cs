...
""";
    ...
    public void RenderDecision_Surface_MatchesGoldenSnapshot()
    {
        ...
        // ─── لِمَ يُطَبَّع الطَرَفان قَبلَ المُقارَنَة ──────────────────────
        // `Golden` نَصّ خامّ داخِل هذا المِلَفّ، و`Capture` تَبني نَظيرَه
        // بِـ`\n` صَريحَة. والمِلَفّ مُودَع بِـLF (‏مَقيسَة: صِفر بايت CR في
        // الـblob)، لكِنّ `core.autocrlf=true` — وهو الافتِراضيّ في إعداد
        // Git for Windows نَفسِه لا في جِهاز بِعَينِه — يُحَوِّلُه إلى CRLF
        // عِندَ السَحب (‏مَقيسَة: ‏327 CR في نُسخَة العَمَل الطازِجَة).
        // فَيَصير `Golden` بِـCRLF والواقِع بِـLF، و`Trim()` لا يَمَسّ إلّا
        // الطَرَفَين فَتَبقى الـCR داخِلَ النَصّ: يَحمَرّ الاختِبار عَلى كُلّ
        // مُستَنسِخ Windows ويَخضَرّ عَلى ubuntu — وهو أَسوَأ مِن العَطَل
        // نَفسِه، لِأَنَّهُ يُدَرِّب المُطَوِّر عَلى تَجاهُل حُزمَة حَمراء.
        //
        // فَالتَّطبيع يُقارِن **المَضمون** لا نِهايات الأَسطُر. وهو حَرفاً
        // ما تَفعَلُه اللَقطَتان الأُختان — `RoleCatalogCharacterizationTests`
        // و`DealsPolicyCharacterizationTests` — وكانَ هذا المِلَفّ الوَحيدَ
        // الشاذَّ عَنهُما مِن الثَلاثَة.
        //
        // ولَم يُعالَج بِتَثبيت المِلَفّ عَلى LF في `.gitattributes` لِأَنَّ
        // ذلكَ يُصَحِّح الاستِنساخَ ولا يُصَحِّح التَأكيد: يَبقى أَيّ لَقطَة
        // ذَهَبيَّة تُكتَب غَداً مَكشوفَةً لِنَفس العِلَّة. والتَثبيت هُناك
        // مَحفوظ لِلَقطات القُرص الَّتي تُقارَن بايتاً بِبايت.
        //
        // ورِسالَة الفَشَل تَطبَع اللَّقطَتَين **بِلا تَطبيع** — الفَرق يُقرَأ
