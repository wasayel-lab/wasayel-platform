...
#
#  والأَنماطُ خَمسَة، وهي **مَوضِعُ السِلسِلَة نَحوِيّاً** لا ذَوقاً
#  (‏docs/I18N.md §٢):
#
...
#     js     — داخِلَ حَرفِيَّةِ سِلسِلَةِ JS في `<script>`
#                                          → @L.Js("KEY")
#              (‏الاقتِباسُ المُحيطُ **يَبقى**، ويُبَدَّل مُحتَواه.
#               ومُحتَوى `<script>` يُصَيَّر خاماً حَتّى بَعدَ أَن
#               يَنقَلِبَ العُنصُر ديناميكِيّاً — قيسَ في الكود
#               المُوَلَّد: `AddMarkupContent` لا `AddContent` —
#               فَهو خارِجَ الفَخّ الثاني وَحدَه بَينَ العَناصِر.)
#
...
# ─── قِراءَةُ المِلَفّ ─────────────────────────────────────────────────
# ‏`:raw` + فَكُّ UTF-8 بِاليَد، لا `:encoding(UTF-8)`. والسَبَبُ مَقيس
# (‏المَوجَة العاشِرَة، §٤-ج): الطَبَقَةُ النَصِّيَّة على Windows
# **تَبتَلِعُ كُلَّ `CR`** (‏315 ← ‏0 في `AgentChatPanel.razor`)،
# فَتَنقَلِب دُفعَةُ تَرحيلٍ مِن عِشرينَ سَطراً إلى مِلَفٍّ كامِلٍ في
# الـdiff — ويَصير الانحِرافُ الصامِتُ غَيرَ مَرئيّ لِمُراجِع. والأَخطَرُ
# أَنّ نِهاياتِ السُطور **بايتاتٌ في المُخرَج** حَيثُ يُكتَب المُحتَوى
# خاماً (‏خاصِّيَّةٌ مُتَعَدِّدَةُ الأَسطُر — الفَخُّ الرابِع — ومُحتَوى
# `<script>`). فَالأَداةُ لا تَلمِس ما لَم تُؤمَر بِتَبديلِه.
sub slurp_utf8 {
    my ($path) = @_;
    open(my $h, '<:raw', $path) or die "لا يُفتَح $path: $!\n";
...
sub spew_utf8 {
    my ($path, $t) = @_;
    utf8::encode($t);
    open(my $h, '>:raw', $path) or die "لا يُكتَب $path: $!\n";
    print $h $t;
    close $h;
}
my $src = slurp_utf8($file);
...
for my $p (sort { $b->{start} <=> $a->{start} } @plan) {
    ...
    if ($p->{mode} eq 'cs') {
        ...
        # داخِلَ حَرفِيَّةِ سِلسِلَةِ JS في `<script>`: الاقتِباسُ المُحيط
        # **يَبقى** (بِخِلاف `cs`) — المُبَدَّلُ مُحتَواه وَحدَه.
        # والحارِسُ يَقولُ لِماذا: عُنصُر `<script>` يُصَيَّر مُحتَواه
        # خاماً حَتّى بَعدَ أَن يَنقَلِب ديناميكِيّاً (‏قيسَ في الكود
        # المُوَلَّد: `AddMarkupContent` لا `AddContent`) — فَهو
        # **خارِجَ الفَخّ الثاني**، والبايتاتُ تُحفَظ إن كانَت القيمَةُ
        # لا تَحتاج هُروباً.
        #
        # والحارِسُ يَفحَصُ **شَرطَ حِفظِ البايتات** لا شَكلَ السِياق:
        # قيمَةٌ فيها ما يَحتاج هُروبَ JS تَخرُج مُبَدَّلَة، فَتُرَدُّ
        # هُنا كَما تُرَدُّ عِندَ البَوّابَة (`value_unsafe_js`).
        die "‏js عِندَ #$p->{idx}: القيمَةُ «$p->{val}» تَحتاج هُروبَ JS "
          . "فَتُبَدِّلُ بايتات الصَفحَة.\n"
            if $p->{val} =~ /['"`\$\\\n\r\x{2028}\x{2029}<>&]/
            || $p->{val} =~ /[\x00-\x1F\x7F-\x9F]/;
        $repl = qq{\@L.Js("$p->{key}")};
    }
    else {
        # ── عُقدَةُ نَصّ ──
