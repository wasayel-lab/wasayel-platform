...
if [ "$EMIT" -eq 1 ]; then
    ...
    echo "#"
    echo "# ── ٣. خارِجَ مُتَناوَل \`L\` — بِانتِظار ADR-001 (‏14) ───────────"
    echo "#"
    echo "# ‏13 في 8 مِلَفّات تَحتَ \`ACommerce.Widgets\` و"
    echo "# \`ACommerce.Templates.Shared\` و\`ACommerce.Kit.Maps.Web\`:"
    echo "#   الحاجِز الأَوَّل — \`L\` يَعيش في مَشروعِ القالِب الَّذي"
    echo "#   **يُرجِعُ إلَيها**، فَحَقنُه فيها يَقلِب اتِّجاهَ الاعتِماد."
    echo "#"
    echo "# و‏1 في FlowExplainer.razor — **داخِلَ مَشروعِ القالِب نَفسِه**:"
    echo "#   الحاجِز الثاني — قيمَةُ مُعامِلٍ افتِراضِيَّة"
    echo "#   (\`[Parameter] … = \"…\"\`) تُنَفَّذ عِندَ الإنشاء **قَبلَ"
    echo "#   الحَقن**. وهذا المَوضِعُ وَحدَه يُثبِت أَنّ الحاجِزَين"
    echo "#   مُستَقِلّان: نُزولُ \`L\` طَبَقَةً لا يُصلِحُه."
    echo "#"
    echo "#   التَفصيل والخِيارات: docs/ADR-001-TEXT-PORT-BELOW-THE-TEMPLATE.md"
    echo "#   الحالَة: **مُقتَرَح، لَم يُنَفَّذ** — لا يُنَفَّذ بِلا إقرار."
    echo "#"
    echo "# ── ٤. المَصرِف الباقي خارِجَ \`.razor\` (‏1) ────────────────────"
    echo "#"
    echo "# SmtpEmailChannel.cs — مَوضوعُ رِسالَة التَحَقُّق."
    echo "#   نَفسُ الحاجِز الأَوَّل: عُدَّةٌ لا تُرجِعُ إلى مَشروعِ القالِب."
    echo "#"
    echo "# ── ٥. نَصٌّ يُصَيَّر ولا يَبلُغُه **أَيُّ** عَدّاد — يُقال ولا يُخفى"
    echo "#"
    echo "# DealTurnView.ActorLabelAr — ‏10 حَرفِيّات (‏6 قيَم مُتَمايِزَة)"
    echo "#   يُصَيِّرُها razor في FlowExplainer وMyDealDetail. دالَّةٌ"
    echo "#   **ساكِنَة** في \`.cs\`: لا العَدّادُ الأَوَّل يَراها (لَيسَت"
    echo "#   razor)، ولا مَصرِفٌ مُعلَنٌ يَلتَقِطُها. بابُها في ADR-001 (د)."
    echo "#"
    ...
    echo "#"
    echo "# ═══ المَجموع: ‏31 في razor (‏13+4+14) و1 مَصرِف، و21 لا يُعَدّ ═══"
