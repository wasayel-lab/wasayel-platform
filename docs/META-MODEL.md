...
### 3.1 بوابة إدارة المستأجر — القراءة والكتابة خلف قرار واحد
...
- شقيقتها `<RequirePlatformAdmin>` تحرس ما هو على نطاق المنصة لا نطاق متجر
  (`/admin`، `/admin/audit`، `/admin/monitor`، `/admin/fixtures`).
